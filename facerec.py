...
def detect_video_faces(gray_frame):
...
def recognize_video_face(model, frame, gray_frame, face_coords, names):
    ...
    for i in range(len(face_coords)):
        ...
        if (confidence<95 and names[prediction] not in recog_names):
            ...
            cv2.putText(frame, names[prediction], (x + 6, y + h - 6), font, 0.5, (0, 0, 0), 1)
            recog_names.append(names[prediction])
            recognized.append((names[prediction]))
        elif (confidence >= 95):
